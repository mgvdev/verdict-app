...
export function RulesTable({
  ...
  rules: {
    ...
    name: string
    description: string
    validation: { negative: number; positive: number }
  }[]
}) {
  return (
    <Table verticalSpacing="xs">
      <Table.Thead>
        <Table.Tr>
          ...
          <Table.Th w={500}>Validation</Table.Th>
        </Table.Tr>
      </Table.Thead>
      <Table.Tbody>
        {rules.map((row) => (
...
function RuleListItem(row: {
  id: number
  name: string
  description: string
  validation: { negative: number; positive: number }
}) {
  // const totalReviews = (row.validation.negative + row.validation.positive) ?? 0;
  ...
  const positiveValidation = 50
  const negativeValidation = 50

  return (
    <Table.Tr key={row.name}>
      <Table.Td>
        <Anchor component="button" fz="sm">
          <Link href={`/rules/${row.id}`}>{row.name}</Link>
        </Anchor>
      </Table.Td>
